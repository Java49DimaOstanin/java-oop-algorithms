...
byte[]array;
	...
	@Disabled
	void test() {
		int size = MemoryService.getMaxAvailibleSize();
		array = new byte[size];
	...
	@Test
	@Disabled
	void allocationMemoryTestTestWithGc() {
		Runtime runtime = Runtime.getRuntime();
		System.out.println("********************");
		System.out.println("Allocation memory map with");
		for(int i = 0 ;i< 10;i++) {
	...
	private void displayMemoryMap(Runtime runtime) {
		System.out.printf("free memory: %dM, total memory: %dM, maximal memory:%dM \n",
				+ runtime.freeMemory()/ MGB,runtime.totalMemory() / MGB,runtime.maxMemory()/ MGB);
	}
}
